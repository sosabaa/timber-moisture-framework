...
5. Add interpreted `LifecycleEvent` / `MoistureRelatedEvent` and `LifecycleIntegrationRecord` nodes where the monitoring record supports maintenance, post-event assessment, repair decisions, or future reuse-related assessment. Record threshold exceedance, moisture dose, drying behaviour, anomaly indicators, inspection actions, and outcomes on the event node, and link fuller event details through `hasEventLog`.
